...
export default function FontCard({ font, fontSize, previewText }: FontCardProps) {
  ...
  const copyToClipboard = (text: string, type: 'link' | 'import') => {
      ...
      <CardFooter className="flex-col items-start gap-4 p-4">
        <div className="flex justify-between w-full">
            <div className="flex gap-4">
                ...
                <a href={font.fileUrl} download onClick={handleDownload} className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-primary text-primary-foreground hover:bg-accent h-9 px-3">
                    <Download className="h-4 w-4" />
                    <span>{downloads.toLocaleString('bn-BD')}</span>
                </a>
            </div>
            <a href={font.fileUrl} download onClick={handleDownload} className="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-primary text-primary-foreground hover:bg-accent h-9 px-3">
                ডাউনলোড
            </a>
        </div>
        <Tabs defaultValue="link" className="w-full">
          ...
          <TabsContent value="link">
            <div className="relative mt-2">
              ...
              <Button size="icon" variant="ghost" className="absolute right-1 top-1/2 -translate-y-1/2 h-7 w-7" onClick={() => copyToClipboard(font.linkUrl, 'link')}>
          ...
          <TabsContent value="import">
            <div className="relative mt-2">
                ...
                <Button size="icon" variant="ghost" className="absolute right-1 top-1/2 -translate-y-1/2 h-7 w-7" onClick={() => copyToClipboard(font.importUrl, 'import')}>
